...
function getTokenForApp(appId: number): string {
  if (API_TOKENS[appId]) return API_TOKENS[appId];
  const appIndex: Record<number, number> = { 303: 0, 304: 1, 305: 2, 306: 3, 307: 4, 308: 5, 309: 6, 310: 7, 311: 8, 312: 9 };
  const index = appIndex[appId];
  if (index !== undefined && COMBINED_TOKENS[index]) return COMBINED_TOKENS[index];
...
async function kintoneRequest(endpoint: string, method: string = 'GET', body?: any, appId?: number): Promise<any> {
  ...
  let targetAppId = appId || body?.app;
  if (!targetAppId) {
    const match = endpoint.match(/app=(\d+)/);
    if (match) targetAppId = parseInt(match[1]);
  }
  ...
  if (!response.ok) throw new Error(data.message || `Kintone error: ${response.status}`);
  return data;
}

export async function getRecords(appId: number, query: string = '', fields?: string[], totalCount: boolean = false): Promise<any> {
...
export async function updateRecord(appId: number, recordId: number, record: any): Promise<{ revision: string }> {
  return kintoneRequest('record.json', 'PUT', { app: appId, id: recordId, record }, appId);
}

// Employee types and operations
...
function mapEmployeeRecord(r: any): Employee {
  return {
    id: parseInt(r.$id?.value || '0'), employeeId: r.employee_id?.value || '',
    email: r.email?.value || '', firstName: r.first_name?.value || '',
    lastName: r.last_name?.value || '', middleName: r.middle_name?.value || '',
    department: r.department?.value || '', position: r.position?.value || '',
    employmentStatus: r.employment_status?.value || '', dateHired: r.date_hired?.value || '',
    birthDate: r.birth_date?.value || '', contactNumber: r.contact_number?.value || '',
    address: r.address?.value || '', emergencyContact: r.emergency_contact?.value || '',
    emergencyNumber: r.emergency_number?.value || '', passwordHash: r.password_hash?.value || '',
    isVerified: r.is_verified?.value === 'Yes', verificationToken: r.verification_token?.value || '',
  };
}
...
export async function getEmployeeById(recordId: number): Promise<Employee | null> {
  try { return mapEmployeeRecord(await getRecord(KINTONE_APPS.EMPLOYEES, recordId)); }
  catch { return null; }
}
...
export async function updateEmployeePassword(recordId: number, passwordHash: string): Promise<void> {
...
export async function setVerificationToken(recordId: number, token: string): Promise<void> {
  await updateRecord(KINTONE_APPS.EMPLOYEES, recordId, { verification_token: { value: token } });
}

export async function updateEmployeeProfile(recordId: number, data: Partial<Employee>): Promise<void> {
  const record: any = {};
...
export async function getEmployeeLeaveRequests(employeeId: string): Promise<LeaveRequest[]> {
  ...
  return response.records.map((r: any) => ({
    ...
    totalDays: parseFloat(r.total_days?.value || '0'), reason: r.reason?.value || '',
    status: r.status?.value || 'Pending', approver: r.approver?.value || '',
    approvedDate: r.approved_date?.value || '', remarks: r.remarks?.value || '', createdAt: r.created_at?.value || '',
  }));
}

export async function createLeaveRequest(employeeId: string, employeeName: string, data: { leaveType: string; startDate: string; endDate: string; totalDays: number; reason: string }): Promise<{ id: string }> {
  return addRecord(KINTONE_APPS.LEAVE_REQUESTS, {
    employee_id: { value: employeeId }, employee_name: { value: employeeName },
    leave_type: { value: data.leaveType }, start_date: { value: data.startDate },
    end_date: { value: data.endDate }, total_days: { value: data.totalDays.toString() },
    reason: { value: data.reason }, status: { value: 'Pending' },
...
export async function getEmployeeDocumentRequests(employeeId: string): Promise<DocumentRequest[]> {
  ...
  return response.records.map((r: any) => ({
    id: parseInt(r.$id.value), employeeId: r.employee_id?.value || '', employeeName: r.employee_name?.value || '',
    documentType: r.document_type?.value || '', purpose: r.purpose?.value || '',
    quantity: parseInt(r.quantity?.value || '1'), status: r.status?.value || 'Pending',
    remarks: r.remarks?.value || '', releaseDate: r.release_date?.value || '', createdAt: r.created_at?.value || '',
  }));
}

export async function createDocumentRequest(employeeId: string, employeeName: string, data: { documentType: string; purpose: string; quantity: number }): Promise<{ id: string }> {
...
export interface Announcement { id: number; title: string; content: string; category: string; priority: string; publishDate: string; expiryDate?: string; isActive: boolean; }

export async function getActiveAnnouncements(): Promise<Announcement[]> {
  const today = new Date().toISOString().split('T')[0];
  const response = await getRecords(KINTONE_APPS.ANNOUNCEMENTS, `is_active = "Yes" and publish_date <= "${today}" order by priority desc, publish_date desc limit 20`);
  if (!response.records) return [];
  return response.records.map((r: any) => ({
    ...
    publishDate: r.publish_date?.value || '', expiryDate: r.expiry_date?.value || '',
...
export async function getEmployeeDTR(employeeId: string, month?: string): Promise<DTRRecord[]> {
  ...
  return response.records.map((r: any) => ({
    ...
    timeIn: r.time_in?.value || '', timeOut: r.time_out?.value || '',
    lunchOut: r.lunch_out?.value || '', lunchIn: r.lunch_in?.value || '',
    totalHours: parseFloat(r.total_hours?.value || '0'), overtimeHours: parseFloat(r.overtime_hours?.value || '0'),
    lateMinutes: parseInt(r.late_minutes?.value || '0'), status: r.status?.value || 'Present', remarks: r.remarks?.value || '',
  }));
}
...
export interface PayrollRecord {
  ...
  taxDeduction: number; loanDeduction: number; otherDeductions: number;
  totalDeductions: number; netPay: number; status: string; payDate: string;
}
...
export interface Benefits {
  employeeId: string; sssNumber: string; philhealthNumber: string; pagibigNumber: string;
  tinNumber: string; hmoProvider: string; hmoPlan: string; hmoCardNumber: string;
  hmoDependents: number; bankName: string; bankAccount: string;
}
...
export async function getEmployeeBenefits(employeeId: string): Promise<Benefits | null> {
  ...
  return {
    ...
    bankName: r.bank_name?.value || '', bankAccount: r.bank_account?.value || '',
  };
}

// Loans operations
export interface Loan {
  id: number; employeeId: string; loanType: string; principalAmount: number;
  interestRate: number; totalAmount: number; monthlyAmortization: number;
  totalPaid: number; balance: number; startDate: string; endDate: string; status: string;
}
